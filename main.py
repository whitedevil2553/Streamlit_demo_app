...
if selected=="Alzhimer Detection":
    ...
    # Load model
    model = CNN_BrainMRI().to(device)
    model.load_state_dict(torch.load("CNN_Alzhimer_4Class_Classification.pt", map_location=device))
    model.eval()
    ...
    if single_image_path is not None:
        ...
        if(st.button("Click For Result")):
            if prediction==0:
                ...
                st.success("Based on the MRI Scanned Image the patient do have a Alzhimer in GLIOMA Region")
            elif prediction==2:
                st.success("Based on the MRI Scanned Image the patient do have a Alzhimer in MENINGIOMA Region")
            elif prediction==3:
                st.success("Based on the MRI Scanned Image the patient do have a Alzhimer in PITUITARY Region")
    else:
        st.write("Import the valid MRI Scanned image of brain witout any unwanted weights occurs!...")
