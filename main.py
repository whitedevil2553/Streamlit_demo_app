...
with st.sidebar:
    selected = option_menu('Multiple Brain Disease Prediction System',
                           ['Brain Tumor Detection','Alzheimer Detection'],
                           menu_icon='hospital-fill',
                           icons=['prescription2','universal-access'],
                           default_index=0)
...
if selected=="Brain Tumor Detection":
    ...
    if single_image_path is not None:
        ...
        left_co, cent_co1,cent_co2,cent_co3,last_co = st.columns(5)
        with cent_co2:
            result_button=st.button("Click For Result")
        ...
        if(result_button):
            if prediction==0:
                ...
                st.error("Based on the MRI Scanned Image the patient do have a tumor in GLIOMA Region")
            elif prediction==2:
                st.error("Based on the MRI Scanned Image the patient do have a tumor in MENINGIOMA Region")
            elif prediction==3:
                st.error("Based on the MRI Scanned Image the patient do have a tumor in PITUITARY Region")
    else:
        st.info("Import the valid MRI Scanned image of brain witout any unwanted weights occurs!...")
...
if selected=="Alzheimer Detection":
    ...
    # Load model
    model = CNN_BrainMRI().to(device)
    model.load_state_dict(torch.load("Alzheimer_4_Class_Classification.pt", map_location=device))
    model.eval()
    ...
    if single_image_path_2 is not None:
        left_co, cent_co1,cent_co2,cent_co3,last_co = st.columns(5)
        with cent_co1:
            image = Image.open(single_image_path_2)  # Open the uploaded image using PIL(pillow library)
            st.image(image, caption="Image uploded Successfully",  width=500)

        #Function call for load and predict image 
        image_tensor = load_and_preprocess_image(single_image_path_2)
        prediction = predict_tumor_presence(model, image_tensor)
    
        #Button for the result
        left_co, cent_co1,cent_co2,cent_co3,last_co = st.columns(5)
        with cent_co2:
            result_button_2=st.button("Click For Result")

        #displaying result
        if(result_button_2):
            if prediction==0:
                st.success("Based on the MRI Scanned Image the patient don't have any kind of Alzheimer")
            elif prediction==1:
                st.error("Based on the MRI Scanned Image the patient do have a MildDemented Alzheimer")
            elif prediction==2:
                st.error("Based on the MRI Scanned Image the patient do have a Moderate-Demented Alzheimer")
            elif prediction==3:
                st.success("Based on the MRI Scanned Image the patient do have a Very-MildDemented Alzheimer")
    else:
        st.info("Import the valid MRI Scanned image of brain witout any unwanted weights occurs!...")
